...
function removeNonRefAdditionalProps(node) {
    ...
    for (const key in node) {
        ...
        if (node.hasOwnProperty(key)) {
            removeNonRefAdditionalProps(node[key]);
        }
    }
}
/**
    ...
    for (let inputIndex = 0; inputIndex < inputs.length; inputIndex++) {
        ...
        reference_walker_1.walkAllReferences(oas, ref => {
            ...
            if (!ref.startsWith('#/')) {
                ...
                if (parts.length !== 2) {
                    throw new Error(`Could not split the reference '${ref}' to two parts`);
                }
                ref = '#' + parts[1];
            }
            return ref;
        });
        removeNonRefAdditionalProps(oas);
    }
    return result;
}
